...
function getDb() {
  if (!globalThis.__antigravityDb) {
    globalThis.__antigravityDb = {
      users: [],
      sessions: [],
      salesHistory: []
    };
  ...
  return globalThis.__antigravityDb;
}
...
module.exports = async (req, res) => {
  const db = getDb();
  const url = new URL(req.url, "https://antigravity.local");
  ...
  try {
    ...
    if (req.method === "POST" && pathname === "/api/forecast") {
      const body = await readBody(req);
      const history = db.salesHistory.filter((row) => row.userId === auth.user.id);
    ...
    if (req.method === "POST" && pathname === "/api/import-sales") {
      ...
      db.salesHistory.push(...rows.map((row) => ({
        ...row,
        id: crypto.randomUUID(),
        userId: auth.user.id
      })));
    ...
    if (req.method === "POST" && pathname === "/api/billing/update") {
      ...
      const userIndex = db.users.findIndex((user) => user.id === auth.user.id);
      db.users[userIndex] = {
        ...db.users[userIndex],
        billing: { ...nextPlan }
      };

      sendJson(res, 200, { billing: db.users[userIndex].billing });
      return;
    }
